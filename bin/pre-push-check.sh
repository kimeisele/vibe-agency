...
# ============================================================================
# CHECK 4: Smoke Tests (fast critical tests only)
# ============================================================================
echo "4️⃣  Running smoke tests (fast critical tests)..."

if ! command -v uv &>/dev/null; then
  echo "   ⚠️  uv not available - skipping smoke tests"
else
  # Run only smoke tests (fast, critical regression checks)
  # Full test suite runs in CI
  if ! uv run pytest \
      tests/test_planning_workflow.py \
      tests/test_coding_workflow.py \
      tests/test_deployment_workflow.py \
      -q --tb=short 2>&1 | tee /tmp/pytest-smoke.log; then
    FAILURES=$(grep -c "FAILED" /tmp/pytest-smoke.log 2>/dev/null || echo "0")
    echo ""
    echo "   ❌ SMOKE TESTS FAILED: $FAILURES test(s) failed"
    echo ""
    echo "   How to fix:"
    echo "     uv run pytest tests/test_*_workflow.py -v    # Run with verbose output"
    echo ""
    echo "   Note: Full test suite runs in CI"
    echo ""
    FAILED=1
  else
    echo "   ✅ Smoke tests passed (full suite runs in CI)"
  fi
fi
...
# ============================================================================
# CHECK 5: Test Coverage (Skip - too slow for pre-push hook)
# ============================================================================
echo "5️⃣  Skipping coverage check (runs in CI)..."
echo "   ℹ️  Coverage report generated by GitHub Actions"
